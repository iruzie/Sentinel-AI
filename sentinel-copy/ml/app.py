...
def log_event(status: str, reason: str, payload: str, request: Request, confidence=None):
    # Prefer X-Forwarded-For so JMeter / proxy-spoofed IPs are logged correctly.
    # Falls back to the real TCP connection IP when the header is absent.
    forwarded_for = request.headers.get("x-forwarded-for")
    client_ip = (
        forwarded_for.split(",")[0].strip()   # take the first (leftmost) address
        if forwarded_for
